...
def retrain_lstm_model():
    ...
    try:
        db = SessionLocal()
        sales_data = db.query(Sales).order_by(Sales.date.asc()).all()
        db.close()

        if len(sales_data) < SEQ_LENGTH + 5:
            print(f"Data tidak cukup untuk training ({len(sales_data)} hari). Minimal {SEQ_LENGTH + 5} hari.")
            return

        all_values = np.array([[getattr(s, m) for m in MENUS] for s in sales_data], dtype=float)
        ...
        for menu in MENUS:
            print(f"Training model {menu}...")
            menu_values = all_values[:, MENUS.index(menu)].reshape(-1, 1)
            ...
            metrics_summary["menus"][menu] = {
                "model_file": f"{menu}_lstm_model.h5",
                "scaler_file": f"{menu}_scaler.pkl",
                "best_params": {
            ...
            print(f"  {menu} selesai | MAE={mae:.4f} RMSE={rmse:.4f} MAPE={mape:.2f}%")
